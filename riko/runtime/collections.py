...
def _settle_iter[T](current: T | None) -> Iterator[Never] | T:
    """Closes a live iterator or creates an exhausted iterator."""
    if current is None:
        result = iter(())
    else:
...
def _passthrough_pipe(
    template: SyncPipe | SyncCollection, source: Items | None = None
) -> SyncPipe:
    """
    Builds an identity ``SyncPipe`` over ``source`` with the template's runtime.

    This preserves the template's execution ``Context`` and settings (parallelism,
    worker pool, ordering) for downstream chaining, but carries no module identity. So
    writing mid-chain never re-runs the preceding module. A shared pipeline pool handle
    transfers cleanup responsibility off the template, exactly as ``_chain`` does.
    """
    ...
    pipe = SyncPipe(
        source=source,
        _pool_handle=shared,
        chunksize=getattr(template, "chunksize", None),
        context=getattr(template, "context", None),
        ordered=getattr(template, "ordered", False),
        parallel=getattr(template, "parallel", False),
        pool_scope=pool_scope,
        threads=getattr(template, "threads", True),
        workers=getattr(template, "workers", None),
    )

    if (
        shared is not None
        and pipe._pool_handle is shared
        and isinstance(template, SyncPipe)
    ):
        template._terminal = False

    return pipe


def _async_passthrough_pipe(
    template: AsyncPipe | AsyncCollection, source: Feed | Awaitable[Feed] | None = None
) -> AsyncPipe:
    """
    Builds an identity ``AsyncPipe`` over ``source`` with the template's runtime.

    This preserves the template's execution ``Context`` and settings (parallelism,
    ordering) for downstream chaining, but carries no module identity. So
    writing mid-chain never re-runs the preceding module.
    """
    return AsyncPipe(
        source=source,
        connections=getattr(template, "connections", DEF_CONNECTION_COUNT),
        context=getattr(template, "context", None),
        ordered=getattr(template, "ordered", False),
        parallel=getattr(template, "parallel", False),
        prefetch=getattr(template, "prefetch", 0),
    )
...
def _write(
    ...
    *,
    ...
    pipe.source = write_through(source, prepared, terminating=lambda: pipe._terminating)
    return pipe


def _awrite(
    source: AsyncPipe | AsyncCollection,
    dest: PathLike,
    *,
    mode: WriteMode | str = WriteMode.REPLACE,
    fmt: FmtLike | None = None,
    ...
    pipe.source = async_write_through(source, prepared)
    return pipe
...
class PyPipe(_Lifecycle):
    ...
    def __init__(
        ...
        source: Feed | Awaitable[Feed] | None = None,
        *,
...
class SyncPipe(PyPipe):
        ...
        **kwargs: object,
        ...
        Args:
            ...
            func: Maps each received item; the subscriber yields its return value,
                so a ``func`` returning ``None`` yields ``None``. For a pure side
                effect prefer ``on_receive``: a ``func`` buffers every item it maps,
                so a side-effect ``func`` retains the whole stream until drained.
                Passing both ``func`` and ``on_receive`` raises ``TypeError``.

            on_receive: A side-effect run on each received item as it arrives; its
                return value is discarded and the item is not buffered, so the
                subscriber drains to nothing. This is what the ``write`` verb uses.
        ...
        register_receiver(
            name, maxlen=maxlen, func=func, on_receive=on_receive, func_kwargs=kwargs
        )
        return receiver
    ...
    def terminate(self) -> None:
        ...
        exit. Unlike ``close``, it does not signal completion to subscribers, so a
        buffered ``write`` discards its partial document rather than saving it as
        if the stream had finished.
        """
    ...
    def _stream(self) -> Stream:
        ...
        try:
            ...
            # publisher still signals DONE to its subscribers; an abrupt terminate
            # (or a real failure below) must not, else a partial/failed publisher
            # looks successfully complete.
            completed = not self._terminating
            raise
        except BaseException:
            self._fail()
    ...
    def split(self, **kwargs: object) -> SyncSplitterWrapperOutput:
        """
        ...
        return cast("SyncSplitterWrapperOutput", splits)
    ...
    def write(
        ...
        *,
        ...
        Examples:
            ...
            ...     flow = SyncPipe(source=items).write(fp.name)
            ...     _ = list(flow)
            ...
        ...
        """
        return _write(self, dest, mode=mode, fmt=fmt)
    ...
    def sink(
        ...
        *,
        ...
        rather than a chainable pipe. A file destination serializes with ``fmt``
        and forbids ``keys``; a keyed record target requires it per its capabilities.
        ...
        Examples:
            ...
            >>>
            >>> with get_temp_file() as fp:
            ...     SyncPipe(source=items).sink(fp.name, mode="replace").written > 0
            True
...
class PyCollection(_Lifecycle):
    ...
    def __init__(
        ...
        **_: object,
        ...
        self.sources: Iterable[InputSource] = sources
...
class SyncCollection(PyCollection):
    ...
    def _stream(self) -> Stream:
        ...
        try:
            ...
            if self.parallel:
                mapped = self.map(fetch_sync_source, zargs, chunksize=self.chunksize)
            else:
                mapped = self.map(fetch_sync_source, zargs)
...
class AsyncPipe(PyPipe):
    ...
    def __await__(self) -> StreamGenerator:
        """
    ...
    def write(
        ...
        *,
        ...
        """
        return _awrite(self, dest, mode=mode, fmt=fmt)
    ...
    async def sink(
        ...
        *,
        ...
        return await _asink(self, dest, mode=mode, keys=keys, fmt=fmt)
    ...
    async def _resolve_source(self) -> AsyncStream | None:
    ...
    async def _materialize_legacy_source[T](
        self, feed: AsyncIterable[T] | None
    ) -> Iterable[T] | None:
        """
    ...
    async def _stream(self) -> AsyncItemGenerator:
        self._begin()
        ...
        mapped_pipeline = cast("Callable[[Item], Awaitable[Stream]]", async_pipeline)
        bounded = self.mapify and self.parallel

        try:
            stream = await self._resolve_source()

            if bounded and stream is not None:
                limit = self.connections
                buffer = self.prefetch
                args = (mapped_pipeline, stream)

                if self.ordered:
                    mapped = async_map_ordered_stream(*args, limit=limit, buffer=buffer)
                else:
                    mapped = async_map_stream(*args, limit=limit, buffer=buffer)
    ...
    async def _stream(self) -> AsyncItemGenerator:
        ...
        try:
            if self.ordered:
                ...
                feeds = (fetch_async_source((src, self.conf)) for src in self.sources)
                merged = async_merge(
...
@overload
def _fetch_source(  # noqa: E704
    args: tuple[InputSource, Conf], pipe: type[SyncPipe]
) -> Stream: ...
@overload  # noqa: E302
def _fetch_source(  # noqa: E704
    args: tuple[InputSource, Conf], pipe: type[AsyncPipe]
) -> AsyncStream: ...
def _fetch_source[T: SyncPipe | AsyncPipe](  # noqa: E302
    args: tuple[InputSource, Conf], pipe: type[T]
) -> Stream | AsyncStream:
...
def fetch_sync_source(
    args: tuple[InputSource, Conf], pipe: type[SyncPipe] = SyncPipe
) -> Stream:
    """Builds a lazy, unstarted iterator over one collection source."""
    return _fetch_source(args, pipe)


def fetch_async_source(
    args: tuple[InputSource, Conf], pipe: type[AsyncPipe] = AsyncPipe
) -> AsyncStream:
    """
...
async def afetch_async_source(
    args: tuple[InputSource, Conf], pipe: type[AsyncPipe] = AsyncPipe
) -> Items:
    """Drains one collection source, for the ordered path that needs it whole."""
    return [item async for item in _fetch_source(args, pipe)]
