...
def is_listlike[T](value: Iterable[T] | object) -> TypeGuard[Iterable[T]]:
    """
    Reports whether a value is listlike (a multi-item iterable) at a boundary.

    A listlike value is any iterable that is not a mapping, primitive, or
    ``None`` (the same boundary ``listize`` uses: mappings and primitives are
    one item, so they are not listlike).

    Args:
        value: The object to classify

    Returns:
        True when value maps over as many items, False when it is one item
