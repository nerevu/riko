...
from riko._io import STREAMING_THRESHOLD, Fetch, seekable
...
def xml2etree(  # noqa: E302
    ...
    if xml:
        ...
        src = seekable(f)
        element_tree = cast("lxmlElementTree", html5parser.parse(src))
    elif IS_LXML:
        ...
        src = seekable(f)
        element = cast("nativeElement", html.parse(src))
        element_tree = cast("nativeElementTree", ElementTree(element))

    if not isinstance(src, str) and src is not f:
        src.close()
