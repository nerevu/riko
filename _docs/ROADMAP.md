...
| Gameplan | Covers |
|---|---|
| [execution-semantics.md](gameplans/execution-semantics.md) | Canonical target runtime: one immutable `Pipeline[T]`; private sync/async executions owning one task group, one exit stack, and one sync/async bridge; immutable `Context` and context-manager resources; `FeedResult`/metadata/provenance; pub/sub/split lifecycle; canonical identity/fingerprints/idempotency; `FeedState`/`StateStore`/checkpoint/CAS; loop iteration; batch mode/backend negotiation; retry/backpressure/timeout/merge/memory semantics. |
