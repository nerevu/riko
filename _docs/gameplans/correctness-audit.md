...
| R11 | **partly fixed** | `dates.py:92` · `_date_utils.py:107` | `tt_to_datedict` derived `_tzinfo` from the tuple but computed `"utime": timegm(tt)`, and `timegm` reads the fields as UTC — an aware `+03:00` timestamp got the 12:00-UTC epoch. **Fixed:** `utime` now builds an aware datetime (`dt(*tt[:5], min(tt[5], 59), tzinfo=_tzinfo or UTC)`) and calls `.timestamp()`; regression in `tests/internal/test_dates.py::test_utime_honors_aware_offset`, verified failing first. **Still open:** `TZINFOS = dict(gen_tzinfos())` snapshots abbreviations **at import**, and duplicates (`CST`) overwrite each other. | C4 · C1 · C8 | A1 | — |
