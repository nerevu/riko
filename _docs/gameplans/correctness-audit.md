...
8. Every § 8 register row is resolved one of three ways: **fixed** (with a regression test per
   #7); **downgraded** to an accepted behaviour pinned by a characterization test that says what
   would change it; or **deferred** to a named owning gameplan — guarded by a strict-xfail
   tripwire that flips to XPASS (failing the suite, forcing its removal) when that owner lands, or,
   where no meaningful tripwire can precede the owning refactor (e.g. a class the refactor deletes),
   recorded as a known-live defect in the merge gate ([release-readiness § 9.1](release-readiness.md#91-merge-gate-features--main)).
...
| R4 | P1 · **partly fixed** | `send.py:80` | `async_parser` buffers into `sent` and returns `iter(sent)` after `complete`; sync `parser` yields per item. Unbounded sources never return, finite ones duplicate the stream in memory — on the one pipe whose purpose is lazy fan-out. **Two adjacent defects fixed 2026-08-24:** (a) `complete` sat *after* the loop, so a `ReceiverUnavailableError` from `publish` skipped it and every healthy receiver blocked forever on a channel nobody would close — now in a `finally`; (b) the loop was a sync `for`, so the `AsyncIterator` that `operator.aparse`/`setup` legitimately produce raised `TypeError: 'async_generator' object is not iterable` — now iterated via `bado.itertools.async_iter`. Both regressions in `tests/public/test_pipe_implementations.py`, verified failing first (a hung to the timeout marker, b raised). **The buffering itself is deferred to F1, not A4** — yielding lazily requires `async_parser` to be an async generator, and the operator wrapper's post-parser path is sync-only *and fails silently*: an async gen isn't awaitable (`_decorators.py:1050`), so `isinstance(stream, Iterator)` is `False`, `get_assignment` (`_assignment.py:110`) takes its `else` branch and emits the generator **object** as a single item. Guard test landed as a `strict` xfail so it flips when F1 lands. | C11 · C12 | ~~A4~~ **F1** | [fanout-topology § 14.1](fanout-topology.md#141-revised-compatibility-mvp-boundary) |
| R5 | P1 | `compile.py:302` | `pythonise` replaces four characters and ASCII-`replace`-encodes, so `"class"`, `"my module"`, `"foo.bar"`, `"1st"`, `"café"` all survive into generated source as identifiers. A pipeline that runs through `build_pipeline` can fail to compile. `ext/codegen.py:59`'s `enum_member_name` already does this properly. | C6 | A5 | [module-enums § one identifier sanitizer](module-enums.md#one-identifier-sanitizer-r5) |
| R6 | ~~P1~~ **fixed** | `_serialize.py:231` | The unsupported check was shallow (`_UNSUPPORTED in hashable_args`) while `_to_hashable` substitutes **recursively**, so `cached({"x": Opaque()})` called the wrapped function with `{"x": _UNSUPPORTED}` and every `Opaque()` collapsed onto one key. Fixed by propagating `_UNSUPPORTED` upward in `_to_hashable` (a container with any unsupported child hashes to `_UNSUPPORTED`), so the shallow call-site check now catches nested cases. Regression in `tests/internal/test_serialize.py`, verified failing first. | C4 | A3 | — |
| R7 | ~~P1~~ **fixed** | `bado/_util.py:68` | `gather_results` returned `[r for r in results if r is not None]`, so a legitimate `None` result was dropped and the list no longer aligned with its inputs. Fixed with the house `MISSING` sentinel (`riko.types._sentinels`): unfilled slots are `MISSING` and the filter is `r is not MISSING`, so `None` results are preserved. No production caller (only the pub/sub collection test); regression in `tests/internal/test_streams.py::test_gather_results_preserves_none_positions`. The eventual replace/delete stays owned by bado-anyio. | C10 | A4 | [bado-anyio § 2](bado-anyio-alignment.md#2-helper-audit-all-confirmed-present-in-the-current-tree) (still slated for replace/delete) |
| R8 | ~~P1~~ **fixed** | `_reencode.py:66` | `read(n)` was `islice(self.stream, n)` — `n` *iterator elements*, not characters/bytes, so `read(1)` over a chunk iterator returned a whole chunk. Fixed with a character/byte remainder buffer: `read(n)` pulls chunks (rejoined with `join_char`) into `self._buffer` until it holds `n` units, returns `buffer[:n]`, and carries `buffer[n:]` forward. `read(0)`→empty and drain (`None`/negative) preserved. Regression in `tests/internal/test_io.py::test_reencode_read_honors_char_count_with_remainder`, verified failing first. | C7 | A2 | — |
| R9 | P1 | `fetchtable.py:91,134` | The module advertises xls(x)/mdb/dbf/sqlite, but both halves open the resource as **text** (`Fetch(url, encoding=…)` / `async_url_open(url, encoding=…)`) before handing it to `meza.io.read`. An xlsx is a zip; decoding it first cannot work. Needs format-dependent binary handling. **Strict-xfail tripwire** landed: `tests/functional/test_basics.py::test_fetchtable_reads_sqlite_fixture` (sqlite half proves the binary-as-text defect; xlsx has no writer in the env), flips to XPASS when connectors.md lands. | C7 | A2 | [connectors.md](connectors.md) |
| R10 | P1 | `fetchdata.py:82,124` · `fetchtable.py:96,137` | `splitext(url)[1]` runs against the whole URL, so `…/export.json?token=abc` mis-detects. Want `splitext(urlparse(url).path)[1]`. Disproportionate for signed/authenticated export URLs — exactly the API-automation direction riko is moving in. **Strict-xfail tripwire** landed: `tests/public/test_fetchdata.py::TestExtensionlessFetchdata::test_query_string_does_not_defeat_extension`, flips to XPASS when connectors.md lands. | C4 | A2 | [connectors.md](connectors.md) |
| R11 | ~~partly fixed~~ **fixed** | `dates.py:92` · `_date_utils.py:107` | `tt_to_datedict` derived `_tzinfo` from the tuple but computed `"utime": timegm(tt)`, and `timegm` reads the fields as UTC — an aware `+03:00` timestamp got the 12:00-UTC epoch. **Fixed:** `utime` now builds an aware datetime (`dt(*tt[:5], min(tt[5], 59), tzinfo=_tzinfo or UTC)`) and calls `.timestamp()`; regression in `tests/internal/test_regressions.py::TestDates::test_utime_honors_aware_offset`, verified failing first. **Second half fixed 2026-09-03:** `gen_tzinfos` no longer snapshots `dt.now()` (it samples a fixed Jan+Jul `2020` instant so both a zone's standard and daylight abbreviation are captured regardless of import date) and resolves ambiguous abbreviations first-wins over a US-centric `_PREFERRED_ZONES` prefix, so `CST` is US Central (UTC-6), not Asia/Taipei (UTC+8). Regressions in `tests/internal/test_regressions.py::TestDates::{test_ambiguous_cst_resolves_to_us_central,test_tzinfos_capture_both_standard_and_daylight_names}`, verified failing first (CST parsed as +8h). | C4 · C1 · C8 | A1 | — |
| R12 | ~~P2~~ **fixed** | `parsers.py:778` | `if text := str(_skip.get("text")):` turned an absent criterion into the literal `"None"`, so `{"content": "none available"}` could be skipped by a rule that specified only `field`. Fixed by reading the raw value first (`text = str(raw) if raw is not None else ""`) so an absent `text` takes the field-presence branch; an explicit empty `text` still does too. Regression in `tests/internal/test_parsers.py::test_get_skip_field_only_follows_presence_not_absent_text`, verified failing first. | C10 | A2 | — |
| R13 | ~~P2~~ **fixed** | `_prepare.py:141` | `if pieces and opts.get("listize")` meant `0`/`False`/`""` stayed scalars under `listize=True`, which the `listize + objectify` path then tried to iterate. Fixed to `if opts.get("listize") and pieces is not None` — branches on the option and special-cases only `None` (preserved, since `listize(None)==[]` may not be wanted here). Regression in `tests/internal/test_prepare.py::test_listize_wraps_falsy_extracted_value`, verified failing first. | C10 | A3 | — |
| R14 | P2 | `timeout.py:99` | `AsyncTimeoutIterator.__anext__` checks the clock either side of `await anext(...)`, so a source that stalls forever is never bounded — the deadline holds only while items keep arriving. Needs a cancel scope carrying the **remaining** deadline. **Strict-xfail tripwire** landed: `tests/internal/test_streams.py::test_timeout_interrupts_a_stalled_source` (asserts elapsed, not content — content is `[{"x": 0}]` both today and post-fix), flips to XPASS when execution-semantics § 7.2 lands. | C11 · C12 | A4 | [execution-semantics § 7.2](execution-semantics.md#72-a-blocked-anext-outlives-the-deadline) |
| R15 | P2 | `bado/io.py:114,131` · `fetchpage.py:96` | `async_url_open` decodes with the caller/default encoding and ignores the HTTP charset; `async_url_read` ignores its explicit `encoding` for HTTP and returns `response.text`; `fetchpage.async_parser` forwards no encoding at all. Four paths decode the same bytes four ways. **Strict-xfail tripwire** landed: `tests/internal/test_io.py::test_async_url_open_honors_content_type_charset` (async-HTTP charset half; `async_url_read`/`fetchpage` halves remain uncovered), flips to XPASS when bado-anyio § 2c lands. | C12 | A4 | [bado-anyio § 2c](bado-anyio-alignment.md#2c-encoding-resolution-precedence-syncasync-parity) |
| R16 | ~~P3~~ **fixed** | `csv.py:93,135` · `fetchtable.py:94,135` | `seekable()` documents that its spooled replacement leaves the original open, but both callers `auto_close`d the spool only — the HTTP response leaked when `has_header=False`. Fixed by making `auto_close` variadic and passing **both** the spool and the original fetch (`auto_close(content, source, f)` / `(…, source, r)`); the leak surfaced only on py314 (`ResourceWarning`). Cross-version regression in `tests/internal/test_io.py::test_csv_headerless_closes_original_source` (spies the original's `close`). | C2 | A2 | — |
| R17 | P3 · **deferred** | `compile.py:936` | *Not reproduced.* `convert_dag` on an empty module list was reported to reach `module_ids[-1]`, but DAG expansion appends the terminal `output` node unconditionally, so it returns just `{_OUTPUT}` with no wires (verified). The paths diverge: the **full-pipe-def** path *does* reject empty input — `parse_pipe_def`/`build_pipeline` on `{"modules": [], "wires": []}` raises `IndexError` at `compile.py:936` (`module_ids[-1]` over an empty topological sort), pinned by `MALFORMED["empty"]` — while the **DAG** path silently yields output-only. **Strict-xfail tripwire** landed: `tests/internal/test_compile.py::test_convert_dag_empty_modules_raises` (asserts `convert_dag({"modules": []})` raises; xfails today, flips when a rejection lands). A clean typed rejection + a single validation front-door that makes both paths reject identically is a schema-validity decision **deferred to the Workflow v2 spec** — its E3.1 `normalize_workflow() -> validate` boundary is that front-door, and E3.2 requires an unambiguous leaf (an empty graph has none), so empty-input rejection is native there; today's `convert_dag`/`_INPUT`/`_OUTPUT` format is the legacy v1 E3.4 normalizes away. Topology is now interpreted once into the immutable `_GraphIndex` (`parse_pipe_def`); the empty case still surfaces `IndexError` via `module_ids[-1]` over the empty `order` (behavior preserved), and that index is the structural seam the E3.1 front-door validates over — see [E3.11](extensibility.md#e311-reuse-of-the-shipped-graph-index). Recalibrate the tripwire from `IndexError` to the spec's chosen error (CLI surface: cli.md `riko pipeline validate`) when it lands. | — | A5 | [extensibility § E3](extensibility.md#e3-canonical-workflow-v2-specification) |
| R18 | P3 | `ext/_resolver.py:39` | `name.startswith(("pipe_", "pipe:"))` routes to the pipeline resolver unconditionally, so a registered **leaf** extension named `pipe_transform` can never resolve through `ModuleRegistry`. Either reserve the prefixes at registration time or try registered modules first. **Strict-xfail tripwire** landed: `tests/internal/test_resolver.py::TestPipeResolver::test_runtime_registered_pipe_prefixed_module_resolves`, flips to XPASS when extensibility § 24 lands. | C7 | A5 | [extensibility § 24](extensibility.md#24-module-registry-and-plugins) |
| R19 | ~~open question~~ **fixed** | `filter.py:76` | `NUMERIC_OPS = {"atmost", "atleast"}` excluded `greater`/`less`, so they compared string operands lexicographically (`"10" < "9"`) — an asymmetry with `atleast`/`atmost`, which already coerced numeric strings. **Decided 2026-09-03 (type-aware, matching Yahoo Pipes' numeric inference + jq/JMESPath):** an `_ordered` comparator wraps the four ordered ops (`greater`/`less`/`atleast`/`atmost`) and compares numerically when **both** operands coerce to `Decimal`, else lexicographically over both as strings — never raises, preserves legitimate string ordering. The characterization test was **updated (not deleted)** into a regression: `tests/public/test_pipe_implementations.py::test_filter_greater_less_compare_numeric_strings_numerically`, verified to differ from the old lexicographic result. Tier 2 (an optional per-rule `type` hint to disambiguate version-like strings, SQL/pandas-style) is a deferred **enhancement**, not a defect. | C4 | A3 | — |
