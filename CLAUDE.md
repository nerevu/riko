...
| `docs/DAG_FORMAT.md` | bare-bones DAG format + `convert-dag`/`compile-pipe` commands |
...
| `docs/ROADMAP.md` | authoritative roadmap and runtime contract (HigherGov-first critical path + RDP/Connect end state + async Feed). Part I runtime-contract sections keep the *design* but their shipped substance is split into `docs/IMPLEMENTED.md`; each Part I status line points to the matching IMPLEMENTED.md section. **Part V (§E0–E8)** = extensibility/ecosystem roadmap (module contract, plugins, workflow schema, observability, adapters, drivers, GUI) folded in from the issue #10 prior-art research; the research record is `docs/lessons.md` (each section tagged with where it landed) |
| `docs/IMPLEMENTED.md` | **as-built companion** to `ROADMAP.md` — the shipped substance of the Part I runtime contract (fully-Implemented §2 + the shipped half of every Partial section), section-numbered 1:1 with ROADMAP Part I. Edit alongside ROADMAP when a Partial→Implemented transition lands |
