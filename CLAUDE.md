...
- **Compiler JSON path** — `build_pipeline`/`_gen_steps` accept a `resolver` (`PipelineResolver`) for sub-pipelines (tests resolve from `tests.pypipelines`); the terminal `output` node is a compiler passthrough (its step is its input stream — there is no `output` module); `gen_modules(embedded=True)` now yields only loop submodules (was: every module); sub-pipelines are called with only their declared kwargs; `_OTHERn` wires aggregate into an `others` list; unresolved leaf modules raise `UnsupportedModuleError` and unresolved `pipe_*` sub-pipelines raise `UnsupportedPipelineError` (import failure, or missing/unreadable JSON under `compile_missing`). Loop/regex-ref handling + subpipeline resolution now compile and run — `test_filtered_multiple_sources` passes (its `xfail` was removed) and the loop tests (`test_loops_1`, `test_loop_example`, `test_urlbuilder_loop`, `test_forever`) are green; the old `test_submodule_loop` was deleted in `908f3a8d`. Fetch URL-list iteration has no covering test (unverified).
