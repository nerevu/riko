...
| `_docs/gameplans/dependency-layers.md` | runtime layer model + dependency-boundary rules (types⇏collections, definition⇏execution, `_derive` leaf, ext resolver ⇏ module-scope compile, new runtime ⇏ compat facade) + the `definition/` regrouping sketch |
...
| `_docs/research/` | prior-art/rationale/ADR notebooks, including preserved prior-project material under `inspiration/`; context only, never authoritative |
...
- **Generated files are never hand-edited** — `riko/types/configs.py` (`gen-config`), `riko/modules/_names.py` + `riko/types/_module_ids.py` (`gen-names`), and the name lists in `_docs/API_SURFACE.md` (`gen-api-surface` / `manage codegen -m api`, rendered from `riko/_api_surface.py` between `<!-- api-surface:KEY -->` markers — surrounding prose stays hand-written); byte-drift guards in `tests/internal/`. Details: `_docs/INTERNALS.md` § Codegen. The `tests/pypipelines/pipe_*.py` + `examples/pypipelines/pipe_*.py` compiled pipes are also generated — from sibling `pipelines/pipe_*.json` via `compile-pipe`; regenerate both trees at once with `gen-pipelines` (`manage codegen -m pipes`), drift-guarded by `test_compile.py::test_codegen_matches_expected_file` (tests) + `test_example_pipes.py` (examples). Fixture maintenance and reconstruction guidance lives in `_docs/INTERNALS.md#compiled-pipeline-fixtures`
- **Docs stay consistent by lint** — `manage lint --docs` guards the `_docs/` model: complete `§0–27` index, active gameplan indexing, version/completion consistency, phase-status ownership, R-phase `ADD`/`MIGRATE`/`DELETE` closure, and the explicit root-doc allowlist. Documentation-authority namespaces are static policy and belong in this linter, not pytest.
- **Don't cite transient scratch docs from stable docs** — root `_docs/*.md` is an explicit lint-enforced authority allowlist. Keep throwaway scratch outside `_docs/`; material worth retaining belongs under `_docs/research/`. Active `_docs/gameplans/*.md` are committed authoritative plans and ROADMAP-indexed; `_docs/research/` and `_docs/archive/` are committed but non-authoritative and never satisfy active ownership.
